...
from chains.serve_milvus import LocalDocQA
...
# get knowledge base list function
def get_vs_list():
    lst_default = ["新建知识库"]
    lst = utility.list_collections()
    lst.sort()
...
# answer question based on loacl knowledge base
local_doc_qa = LocalDocQA()

# gradio
flag_csv_logger = gr.CSVLogger()

def get_answer(query, vs_name, history, mode, score_threshold=VECTOR_SEARCH_SCORE_THRESHOLD,
    ...
    if mode == "Bing搜索问答":
        for resp, history in local_doc_qa.get_search_result_based_answer(
            ...
            history[-1][-1] += source
            yield history, ""
    elif mode == "知识库问答" and vs_name is not None:
        print("****************************")
        print(vs_name)
        print("****************************")
        for resp, history in local_doc_qa.get_knowledge_based_answer(
                query=query, vs_name=vs_name, chat_history=history, streaming=streaming):
            source = "\n\n"
            source += "".join(
        ...
        if os.path.exists(vs_name):
            resp, prompt = local_doc_qa.get_knowledge_based_conent_test(query=query, vs_name=vs_name,
        ...
        for answer_result in local_doc_qa.llm.generatorAnswer(prompt=query, history=history,
            ...
            yield history, ""
    logger.info(f"flagging: username={FLAG_USER_NAME},query={query},vs_name={vs_name},mode={mode},history={history}")
    flag_csv_logger.flag([query, vs_name, history, mode], username=FLAG_USER_NAME)
...
def get_vector_store(vs_id, files, sentence_size, history, one_conent, one_content_segmentation):
    ...
    if local_doc_qa.llm and local_doc_qa.embeddings:
        if isinstance(files, list):
            ...
            vs_name, loaded_files = local_doc_qa.init_knowledge_vector_store(filelist, vs_name, vs_list, sentence_size)
        else:
            vs_name, loaded_files = local_doc_qa.one_knowledge_add(vs_name, files, one_conent, one_content_segmentation,
                                                                   sentence_size)
        if len(loaded_files):
    ...
    return vs_name, None, history + [[None, file_status]]
...
def change_vs_name_input(vs_id, history):
    if vs_id == "新建知识库":
        ...
        return gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), os.path.join(VS_ROOT_PATH,
                   ...
                   [None, file_status]]
        '''
        return gr.update(visible=False), gr.update(visible=False), gr.update(visible=True), vs_id, history + [[None, file_status]]
