...
class LocalDocQA:
    ...
    def init_knowledge_vector_store(self,
        ...
        if len(docs) > 0:
            ...
            if vs_name in vs_list:
                ...
                # if not vs_name:
                    # vs_name = str(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    ...
    def one_knowledge_add(self, vs_name, one_title, one_conent, one_content_segmentation, sentence_size):
        embedding_model = EMBEDDING_MODEL
        embedding_device=EMBEDDING_DEVICE
        self.embeddings = HuggingFaceEmbeddings(model_name=embedding_model_dict[embedding_model],
                                                model_kwargs={'device': embedding_device})
        try:
            if not vs_name or not one_title or not one_conent:
    ...
    def get_knowledge_based_answer(self, query, vs_name, chat_history=[], streaming: bool = STREAMING):
        embedding_model = EMBEDDING_MODEL
        embedding_device = EMBEDDING_DEVICE
        self.embeddings = HuggingFaceEmbeddings(model_name=embedding_model_dict[embedding_model],
                                                model_kwargs={'device': embedding_device})
    ...
    def get_knowledge_based_conent_test(self, query, vs_name, chunk_conent,
...
if __name__ == "__main__":
    # 初始化消息
    # args = None
    args = parser.parse_args(args=['--model-dir', '/data/ssd/xsl/models/', '--model', 'chatglm-6b', '--no-remote-model'])
    # args = parser.parse_args(args=['--model', 'moss'])
